...
print('Convertendo Data Frame em matriz com dados transformados com LabelEncoder')
X=df.apply(LabelEncoder().fit_transform).iloc[:,0:8].values.astype(str)
print(X)

print('Convertendo Data Frame em matriz com dados originais')
X2=df.iloc[:,0:8].values.astype(str)
print(X2)
print('')
...
for index, item in enumerate(X):
    ...
    print("Nova Instância", index, ':', X2[index], "- Previsão:", previsao, '-', DescPrevisao(previsao[0]))
